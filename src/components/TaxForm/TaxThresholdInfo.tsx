        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-4">
          {TAX_FREE_THRESHOLDS.map((threshold, index) => (
            <div key={index} className="bg-green-50 p-4 rounded-lg border border-green-200">
              <h4 className="font-semibold text-green-800">{t.taxThresholdData?.[threshold.familyTypeKey as keyof typeof t.taxThresholdData] || threshold.familyTypeKey}</h4>
              <div className="text-2xl font-bold text-green-600 my-2">
                {formatCurrency(threshold.threshold)}
              </div>
              <div className="text-xs text-gray-600">{t.taxThresholdData?.[threshold.calculationKey as keyof typeof t.taxThresholdData] || threshold.calculationKey}</div>
            </div>
          ))}
        </div>
      </CardContent>
